(function () {
  ...
  (async function () {
    ...
    const titleName    = "PUTRAMODZ BYPASS";
    const telegramLink = "https://t.me/elvnhx";
    ...
    styleEl.textContent = `
      ...
      .vanz-btn-instant-dewa { border-right: 4px solid #aa00ff; }
      .vanz-btn-instant-dewa:hover { background: #aa00ff; color: #fff; box-shadow: 0 0 25px rgba(170,0,255,0.5); }
    `;
          ...
          try {
            ...
            const timer = setInterval(() => {
              ...
              if (remaining <= 0) {
                ...
                // Simulasi perilaku natural sebelum redirect agar tidak terdeteksi anomaly
                const naturalDelay = Math.floor(Math.random() * 1500) + 1000; // 1000-2500ms random
                
                // Simulasi mouse movement event
                try {
                  const me = new MouseEvent("mousemove", { bubbles: true, cancelable: true,
                ...
                // Simulasi scroll kecil
                try { window.scrollBy(0, Math.floor(Math.random() * 5)); } catch(e) {}

                countdownOverlay.remove();

                setTimeout(() => {
                  // Buka via anchor click (lebih natural dari location.replace)
                  try {
      ...
      if (isValid) {
        ...
        setTimeout(() => {
          authBox.innerHTML = `
            ...
            <button id="vanz-btn-instant" class="vanz-mode-btn-dewa vanz-btn-instant-dewa">00 // INSTANT_MODE [1S_WARP]</button>
          ...
          `;
          document.getElementById("vanz-btn-instant").addEventListener("click", () => runRedirect(1));
